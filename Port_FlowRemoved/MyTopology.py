...
# receiving link events.
    ...
    def _handle_LinkEvent (self, event):
        ...
        if event.added:
            ...
            check = self.findLink(rcvd_link.dpid1, rcvd_link.port1)
            if check:
    ...
    def findLink(self, sw1 = None, port1 = None):
        for l in self.Links:
            if l.sw1 == sw1 and l.port1 == port1:
                return l
        return None # if not found

    def findLinkByReceiver(self, sw2 = None, port2 = None):
        for l in self.Links:
            if l.sw2 == sw2 and l.port2 == port2:
                return l
        return None # if not found

    def getLinkCost(self, sw1 = None, port1 = None, sw2 = None, port2 = None):
        for l in self.Links:
            if l.sw1 == sw1 and l.port1 == port1 and l.sw2 == sw2:
                # log.debug("Found link cost. Returning: %s", str(l.link_cost))
                return l.link_cost
        return None # if not found
    ...
    def calculateLoads(self, stamp):
        ...
        for l in self.Links:
            ...
            # if l.cur_counter >= l.prev_counter: else l.last_load = (max(longlong)-prev_counter)+l.cur_counter
            # Previous value might be invalid as counters increment from LLDP bytes. Nonetheless we can solve this by
            # sending port stats requests to all switches every 100 polling periods for example.
            ...
            # log.debug("%s | %s Link Counters after--> Prev: %i, Current: %i, Load: %i, link_cost: %i", l.sw1,l.sw2,l.prev_counter,l.cur_counter,l.last_load, l.link_cost)
