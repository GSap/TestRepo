...
Active Flows are tracked. When a PacketIn is received it activates the flow and when a flow removed is received from the
first switch in the path of the active flow, the flow is deactivated.
...
and send Flow Mods again. --> reactive scenario

CalculatePath(): It should work like Djikstra but every link has a cost equal to its' load. So a 40% loaded link has cost of 40.
The highest cost link is chosen as best. Links with cost over 49 are not taken into account unless no other path exists.
...
class MyForwarding (object):
    ...
    def _handle_FlowRemoved (self, event):
        # monitor active flows: deactivate flow in active flow list. Set higher priority to monitoring module so that it
        # can update link info and TM before deleting the entry from here.
        ...
        for flow in self.ActFlows:
            if flow.flow_match.s_ip == srcip and flow.flow_match.d_ip == dstip:
                log.debug("Found flow from %s to %s that has to be removed from Active flows list", srcip, dstip)
                flow.active = None
                log.debug("Marked flow as inactive in Active Flows list")
    ...
    def sendForwardingRule(self, srcip, dstip, bid, out_port, dpid):
        # method that sends packet to one switch only
        msg = of.ofp_flow_mod()
        ...
        for af in self.ActFlows: # find the flow in active flows
            if msg.match.nw_src == af.flow_match.s_ip and msg.match.nw_dst == af.flow_match.d_ip and af.active:
                if dpid_to_str(dpid) in af.used_path[0]:
    ...
    def setUpPath(self, srcip, dstip, bid, dpid):
        # method that sets up the forwarding path for a new incoming flow
        for af in self.ActFlows: # find the flow in active flows
            if toSubnet(srcip) == af.flow_match.s_ip and toSubnet(dstip) == af.flow_match.d_ip and af.active:
